...
select
  ...
  coalesce(nullif(replace(btrim(l.phone), '''', ''), ''), 'UNKNOWN-' || l.legacy_row_id::text) as phone,
  ...
  coalesce(public.try_parse_timestamptz(l.created_at_raw), timezone('utc', now())) as created_at,
